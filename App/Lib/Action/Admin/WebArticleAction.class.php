...
class WebArticleAction extends AdminAction{
    ...
    function index(){
		...
		$group = '<select name="catid">
			...
			$grouplist = M("yunweb_artcat")->order("listorder")->select();
			if(!empty($grouplist))
			{
    ...
    function del() {
        if ($_GET['id'] != "") {
            M("yunweb_article")->where("id=" . $_GET['id'])->delete();
        }
	...
	function add()
	{
        ...
        if ($_POST['dopost'] == "save")
		{
           ...
           $address =  M("yunweb_article")->add($addmap);
        ...
        $group = '<select name="catid">
            <option value="">请选择</option>';
            $grouplist = M("yunweb_artcat")->select();
            if(!empty($grouplist))
            {
                foreach($grouplist as $key=>$val)
                {
	...
	function edit()
	{
		...
		if ($_POST['dopost'] == "save")
		{
			...
			if(!empty($_POST['image'])){
				$image = ExecUpload($_POST['image'], $_POST['old_image'], $fileurl);
				 M("yunweb_article")->where(array("id"=>$id))->save(array("thumb"=>$image));
			}
			...
			$result = M("yunweb_article")->where(array("id"=>$id))->data($addmap)->save();
		...
		$info = M("yunweb_article")->where(array("id"=>$id))->find();
	...
	function artopened()
	{
		...
		if(!empty($id))
		{
			M("yunweb_article")->where(array("id"=>$id))->data(array("status"=>1))->save();
	...
	function artdisabled()
	{
		...
		if(!empty($id))
		{
			M("yunweb_article")->where(array("id"=>$id))->data(array("status"=>0))->save();
    ...
    {
        $id = intval($_REQUEST['id']);
        $is_recommend_s = M("yunweb_article")->where("id = $id")->getfield("is_recommend");
        $is_recommend = ($is_recommend_s=='0') ? "1" : '0';
        $res = M("yunweb_article")->where(array("id"=>$id))->data(array("is_recommend"=>$is_recommend))->save();

...
class WebArtlanmuAction extends AdminAction{
    ...
    function del() {
        if ($_GET['cid'] != "") {
            M("yunweb_lanmu")->where("id=" . $_GET['cid'])->delete();
        }
	...
	function edit()
	{
        ...
        if($id){
            ...
            $pid = M("yunweb_lanmu")->where($where)->getfield('pid'); 
		...
		{	
            ...
            $posi_res = M("yunweb_lanmu")->where(array("id"=>$id))->data($posidarr)->save();
            $this->success("修改成功！", U("index"));
			exit;
		}
	
   		$info = M("yunweb_lanmu")->where(array("id"=>$id))->find();	
        ...
        foreach($data as $k => $v)
        {
           ...
           { 
            ...
            $html .= '<tr class="tr_white">
                            ...
                            <td align="center">
                            <input class="btn btn-white btn-sm" onclick="window.location.href ='."'/admin/web_article/add/cid/$v[id]'".'" value="添加内容" type="button">
                            <input class="btn btn-white btn-sm" onclick="window.location.href ='."'/admin/web_artlanmu/add/cid/$v[id]'".'" value="添加子栏目" type="button">
                            <input class="btn btn-white btn-sm" onclick="window.location.href ='."'/admin/web_artlanmu/edit/cid/$v[id]'".'" 
    ...
    function selecttree($id){  
        $data = M("yunweb_lanmu")->where($where)->order("listorder")->select(); 

...
class WebNavAction extends AdminAction{
    ...
    function del(){
    	$id=$_REQUEST['id'];
        if ($id != "") {
        	$pidlist =  M("yunweb_nav")->where("pid=".$id)->select();

        	if($pidlist){
        		$this->success("请删除分类下子栏目继续操作", U("index"));exit();
        	}else{
        		M("yunweb_nav")->where("id=".$id)->delete();
    ...
    function headeradd()
	{
		...
		$pidlist =  M("yunweb_nav")->where($where)->order("id asc")->select();
	...
	function edit()
	{
		...
		if ($_POST['dopost'] == "save")
		{
            ...
            $res = M("yunweb_nav")->where(array("id"=>$id))->data($addmap)->save();
			
			$this->success("修改成功！", U("index"));
            exit();
		}
		$info = M("yunweb_nav")->where(array("id"=>$id))->find();
	...
	{
		$id = intval($_REQUEST['id']);
        $is_recommend_s = M("yunweb_nav")->where("id = $id")->getfield("isshow");
        $is_recommend = ($is_recommend_s=='0') ? "1" : '0';
        $res = M("yunweb_nav")->where(array("id"=>$id))->data(array("isshow"=>$is_recommend))->save();
	...
	{
		$id = intval($_REQUEST['id']);
        $is_recommend_s = M("yunweb_nav")->where("id = $id")->getfield("isblank");
        $is_recommend = ($is_recommend_s=='0') ? "1" : '0';
        $res = M("yunweb_nav")->where(array("id"=>$id))->data(array("isblank"=>$is_recommend))->save();
        echo json_encode($is_recommend);die;
	}
	...
	function getTree($data, $pId,$n)
    {
        ...
        foreach($data as $k => $v)
        {
           ...
           { 
		    ...
		    $del='<input class="btn btn-white btn-sm" onclick="window.location.href ='."'/admin/web_artlanmu/del/cid/$v[id]'".'"  value="删除"   type="button">';
            ...
            $html .= '<tr class="tr_white">
                            ...
                            '.$v['outlink'].'
                               ...
                               <img  id="img2_'.$v['id'].'"  src="__PUBLIC__/admin/images/'.$blankgif_img.'" onclick="set_blank('.$v['id'].');" > 
                                ...
                                <td align="center">
							<input class="btn btn-white btn-sm" onclick="window.location.href ='."'/admin/WebNav/edit/id/$v[id]'".'" value="修改" type="button">
							<input class="btn btn-white btn-sm" onclick="window.location.href ='."'/admin/WebNav/del/id/$v[id]'".'" value="删除" type="button">
                            </td>';

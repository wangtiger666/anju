...
class WebAdAction extends AdminAction{
    ...
    function index(){
        ...
        foreach ($list as $k=>$li) {
            $panocount = M("yunweb_adlist")->where(array("member_id"=>$li['id']))->count();
	...
	function add()
	{
		if ($_POST['dopost'] == "save")
		{
            ...
            M("yunweb_adlist")->add($addmap);
			$this->success("添加成功！");
			exit;
		}

		$grouplist = M("yunweb_ad")->select();
   ...
   function del() {
        ...
        if ($_GET['id'] != "") {
            M("yunweb_adlist")->where("id=" . $_GET['id'])->delete();
        }
        $this->success("删除成功！");
    }
...
public function upload(){
		...
		{	
			...
			if(!empty($_POST['image'])){
				$image = ExecUpload($_POST['image'], $_POST['old_image'], $fileurl);
				 M("yunweb_adlist")->where(array("id"=>$id))->save(array("thumb"=>$image));
			}
            ...
            M("yunweb_adlist")->where(array("id"=>$id))->data($save_date)->save();
            $this->success("修改成功！");
			exit;
        
		}

		$info = M("yunweb_adlist")->where(array("id"=>$id))->find();
	...
	function adopened()
	{
		...
		if(!empty($id))
		{
			$adid = M("yunweb_adlist")->where(array("id"=>$id))->getField('ad_id');
            M("yunweb_adlist")->where(array("id"=>$id))->data(array("is_show"=>1))->save();
			$this->success("显示成功！");
		}
	}
	...
	function addisabled()
	{
		...
		if(!empty($id))
		{
			M("yunweb_adlist")->where(array("id"=>$id))->data(array("is_show"=>0))->save();
			$this->success("禁用成功！");
		}
	}

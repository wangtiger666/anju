...
class YunArtdanyeAction extends AdminAction{

    function index(){
        cookie("back", __SELF__);
        import('ORG.Util.Page'); // 导入分页类

        $mm = M("yun720_artdanye");
    ...
    function del() {
        if ($_GET['id'] != "") {
            M("yun720_artdanye")->where("id=" . $_GET['id'])->delete();
        }
	...
	function add()
	{
        ...
        if ($_POST['dopost'] == "save")
		{
           ...
           $address =  M("yun720_artdanye")->add($addmap);
	...
	function edit()
	{
		...
		if ($_POST['dopost'] == "save")
		{
			...
			if(!empty($_POST['image'])){
				$image = ExecUpload($_POST['image'], $_POST['old_image'], $fileurl);
				 M("yun720_artdanye")->where(array("id"=>$id))->save(array("thumb"=>$image));
			}
			...
			$result = M("yun720_artdanye")->where(array("id"=>$id))->data($addmap)->save();
		...
		$info = M("yun720_artdanye")->where(array("id"=>$id))->find();
	...
	function artopened()
	{
		...
		if(!empty($id))
		{
			M("yun720_artdanye")->where(array("id"=>$id))->data(array("status"=>1))->save();
	...
	function artdisabled()
	{
		...
		if(!empty($id))
		{
			M("yun720_artdanye")->where(array("id"=>$id))->data(array("status"=>0))->save();

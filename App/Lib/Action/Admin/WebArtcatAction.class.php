...
class WebArtcatAction extends AdminAction{
    ...
    function del() {
        ...
        if ($_GET['id'] != "") {
            M("yunweb_artcat")->where("id=" . $_GET['id'])->delete();
        }
	...
	function add()
	{
		if ($_POST['dopost'] == "save")
		{
            ...
            $res = M("yunweb_artcat")->add($addmap);
	...
	function edit()
	{
		...
		{	
            ...
            $posi_res = M("yunweb_artcat")->where(array("id"=>$id))->data($posidarr)->save();
   		...
   		$info = M("yunweb_artcat")->where(array("id"=>$id))->find();

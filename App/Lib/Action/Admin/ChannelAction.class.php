...
class ChannelAction extends AdminAction{
    ...
    function del() {
        ...
        if ($_GET['id'] != "") {
            M("yun_channel")->where("id=" . $_GET['id'])->delete();
        }
	...
	function edit()
	{
		...
		{	
            ...
            $posi_res = M("yun_channel")->where(array("id"=>$id))->data($posidarr)->save();
   		...
   		$info = M("yun_channel")->where(array("id"=>$id))->find();

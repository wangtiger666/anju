<?php
class Ajax3dAction extends MemberAction {
    ...
    public function pano() {
        ...
        if (I("post.dopost") == "one2six") {
		...
		else if (I("post.dopost") == "ball3six") {
            $photofile = APP_ROOT . I("post.photofile");
            if (is_file($photofile)) {
                $actiondir = APP_ROOT . "/uploads/user/" . substr(md5($this->member_id), 5, 15) . "/station";
                $balldir = APP_ROOT . "/Public/krpano";
                RemoveDirFiles($actiondir);
                createFolder($actiondir);
                //copyFolder($balldir, $actiondir);
                $bs = explode(".", basename($photofile));
				...
				//$file = MoveViewPhoto3d($file, $this->member_id, $pano_id, $filedir, $pk);
				...
				exec($balldir.'/krpanotools64.exe register '.$register);
				exec($balldir.'/krpanotools32.exe register '.$register);
                rename($photofile, $file);
				
				
				$file = MoveView3dPhoto($file,$this->member_id);
				//echo $file;
				//sleep("5");
				
				
				
                if (exec($balldir . '/sphere3cube.bat ' . $file, $output)) {
                    echo "ball3sixdo(0);";
                }
				exec($balldir . '/cut.bat ' . $file, $output);
            } else {
                echo "showMsg('文件不存在！',2);";
            }
        } else if (I("post.dopost") == "ball3sixdo") {
            $actiondir = APP_ROOT . "/uploads/user/" . substr(md5($this->member_id), 5, 15) . "/station";
            $webdir = "/uploads/user/" . substr(md5($this->member_id), 5, 15) . "/station";
            $n = I('post.n');
            $member_id = $this->member_id;
            ...
            if ($n < 6) {
                ...
                $temp = $this->pano[$n];
                $saveurl = $webdir . "/pano_1_{$this->mbpano[$n]}.jpg";
                $weburl = CC("web_root") . $webdir . "/pano_1_{$this->mbpano[$n]}.jpg";
            ...
            if ($os == "linux") {
                ...
                if ($n == 0) {
                    ...
                    //$file = "";
                    //$file .= "' {$actiondir}/pano_1_u.jpg'";
                    //$file .= " '{$actiondir}/pano_1_d.jpg'";
                    //$file .= " '{$actiondir}/pano_1_l.jpg'";
                    //$file .= " '{$actiondir}/pano_1_r.jpg'";
                    //$file .= " '{$actiondir}/pano_1_b.jpg'";
                    //$file .= " '{$actiondir}/pano_1_f.jpg'";
					//$file .= " '{$actiondir}/pano_2_u.jpg'";
                    //$file .= " '{$actiondir}/pano_2_d.jpg'";
                    //$file .= " '{$actiondir}/pano_2_l.jpg'";
                    //$file .= " '{$actiondir}/pano_2_r.jpg'";
                    //$file .= " '{$actiondir}/pano_2_b.jpg'";
                    //$file .= " '{$actiondir}/pano_2_f.jpg'";
                    $n++;

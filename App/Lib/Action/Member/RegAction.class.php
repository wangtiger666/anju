    ...
    function index() {
        if (I("post.dopost") == "reg") {
            ...
            if ($account == "") {
                ...
                if (is_array($row)) {
                    echo "javascript:alert('帐号已经存在！',2);";
                    exit();
                }
            }
            if ($pwd == "") {
        ...
        $weixinlogin_url = "https://open.weixin.qq.com/connect/qrconnect?appid=$appid&redirect_uri=http://vr.mlabc.com/member/login/weixinlogin&response_type=code&scope=snsapi_login&state=STATE#wechat_redirect";
...
function send_mobile_code(){
 ...
 function sendjuheSMS($mobile, $content){
    //$appkey = M("dxconfig")->where('id=2')->getField('appid');
    $appkey =M("dxconfig")->where('id=1')->getField('key');
    $tpl_id=M("dxconfig")->where('id=1')->getField('mobanid');
